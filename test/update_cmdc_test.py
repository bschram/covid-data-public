...
from scripts.update_covid_county_data import Covid_County_DataTransformer, DATA_ROOT
import requests_mock

from scripts.helpers import UNEXPECTED_COLUMNS_MESSAGE


def test_update_covid_county_data():
    ...
    with structlog.testing.capture_logs() as logs, requests_mock.Mocker() as m:
        ...
        # TODO(tom): Pass in apikey when https://github.com/valorumdata/covid_county_data.py/issues/9 is fixed.
        # Same in other tests.
        transformer = CovidCountyDataTransformer.make_with_data_root(
            DATA_ROOT, None, structlog.get_logger()
        )
